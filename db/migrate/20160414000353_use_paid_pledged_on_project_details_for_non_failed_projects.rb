...
SELECT p.id AS project_id,
  ...
  GROUP BY p.id, fp.id, c.id, u.id, c.name_es, ct.name, u.address_city, st.acronym, u.address_state, st.name, pt.progress, pt.pledged, pt.paid_pledged, pt.total_contributions, p.state, p.expires_at, pt.total_payment_service_fee, fp.state, pt.total_contributors;
    SQL
  end
