...
AS $$
    ...
    BEGIN
        ...
        full_text_index :=  setweight(to_tsvector('english', unaccent(coalesce(project.name::text, ''))), 'A') ||
                            ...
                            setweight(to_tsvector('english', unaccent(coalesce((SELECT c.name_es FROM categories c WHERE c.id = project.category_id)::text, ''))), 'B') ||
...
CREATE OR REPLACE FUNCTION public.update_full_text_index() RETURNS trigger
    ...
    BEGIN
      ...
      new.full_text_index :=  new.full_text_index || setweight(to_tsvector('english', unaccent(coalesce((SELECT c.name_es FROM categories c WHERE c.id = NEW.category_id)::text, ''))), 'B');
      RETURN NEW;
    END;
    $$;
